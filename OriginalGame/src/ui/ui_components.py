...
import pygame
from ..constants import (WHITE, BLACK, GREY, RED, BLUE, HIGHLIGHT, BACKGROUND, 
...
class Button:
    ...
    """
    def __init__(self, x, y, width, height, text, color=WHITE, hover_color=GREY, text_color=BLACK):
        """
        Initialize a button.
        """
...
class PieceCounter:
    ...
    def draw(self, screen, piece_counts):
        ...
        for i, size in enumerate(range(3)):
            count = piece_counts.get(size, 0)
            label = f"{labels[i]}: {count}"
...
class Timer:
    ...
    def draw(self, screen):
        ...
        screen.blit(turn_surface, (self.x, self.y + 25))    
